...
1. **Плейсхолдеры путей.** В `core/**` никогда не хардкодьте пути инструментов
   (`.kilo/`, `.claude/`, `.opencode/`). Используйте `{{CONTEXT_DIR}}`, `{{LOGS_DIR}}`,
...
2. **Правильное имя адаптера** — **OpenWork** (`openworks`); продукт OpenWork построен на
   OpenCode primitives и читает `.opencode/agents/*.md` + `opencode.jsonc`. Целевые пути
   установки: `.opencode/agents`, `.opencode/skills`, `.opencode/context`.
...
5. **Атрибуция стороннего.** При добавлении чужих материалов сохраняйте copyright-заголовки и
   строку `# Source:`; обновляйте `NOTICE.md` и `THIRD_PARTY_LICENSES.md`. Оригинальная часть
   harness — под лицензией MIT (Copyright (c) 2025-2026 Kirill Pulyavin, см. `LICENSE`); новый
   оригинальный материал автоматически попадает под MIT.
