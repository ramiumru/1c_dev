...
2. **Классифицировать запрос:**
   ...
   - «дай текст запроса 1С / напиши пример BSL» (пользователь выполнит сам; правки исходников
     не просят) → intent `artifact`: роль `1c-analyst`; для BSL/запрос-специфики — роль
     `1c-developer` в read-only artifact-режиме (текст артефакта в ответе; БЕЗ правки
     исходников и apply; отсутствие src — не ошибка).
