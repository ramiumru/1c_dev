...
8. **Проверка результата.** Сверить фактически изменённые файлы с планом (через `read`/`grep`
   ...
   на изменённые `.bsl` (статическая проверка — не доверять безусловно самоотчёту
   `1c-developer`), запустить доступные безопасные тесты, сформировать отчёт о фактически
   применённых изменениях.
9. **Успех** → (опц., по брифу) `db-run` открыть Предприятие.
10. Ответ по формату ниже.
