...
> **Source of truth — repository, не БД.** Эта выгрузка предназначена для экспорта/бэкапа
> в **отдельный каталог**. **НЕ выгружайте БД поверх `projects/<источник>/src/**`** и не
> используйте этот скилл для «синхронизации» repository из БД: состояние устаревшей БД не
> должно затирать repository. Направление восстановления — только repository → БД
> (`db-load-xml`).
...
```powershell
powershell.exe -NoProfile -File "{{SKILL_DIR}}/scripts/db-dump-xml.ps1" <параметры>
```
...
powershell.exe -NoProfile -File "{{SKILL_DIR}}/scripts/db-dump-xml.ps1" -V8Path "C:\Program Files\1cv8\8.3.25.1257\bin" -InfoBasePath "C:\Bases\MyDB" -UserName "Admin" -ConfigDir "C:\WS\cfsrc" -Mode Full

# Инкрементальная выгрузка
powershell.exe -NoProfile -File "{{SKILL_DIR}}/scripts/db-dump-xml.ps1" -InfoBasePath "C:\Bases\MyDB" -UserName "Admin" -ConfigDir "C:\WS\cfsrc" -Mode Changes

# Частичная выгрузка
powershell.exe -NoProfile -File "{{SKILL_DIR}}/scripts/db-dump-xml.ps1" -InfoBasePath "C:\Bases\MyDB" -UserName "Admin" -ConfigDir "C:\WS\cfsrc" -Mode Partial -Objects "Справочник.Номенклатура,Документ.Заказ"

# Серверная база
powershell.exe -NoProfile -File "{{SKILL_DIR}}/scripts/db-dump-xml.ps1" -InfoBaseServer "srv01" -InfoBaseRef "MyApp_Dev" -UserName "Admin" -Password "secret" -ConfigDir "C:\WS\cfsrc" -Mode Full

# Выгрузка расширения
powershell.exe -NoProfile -File "{{SKILL_DIR}}/scripts/db-dump-xml.ps1" -InfoBasePath "C:\Bases\MyDB" -UserName "Admin" -ConfigDir "C:\WS\ext_src" -Mode Full -Extension "МоёРасширение"
```
