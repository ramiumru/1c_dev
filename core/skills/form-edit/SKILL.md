...
```powershell
powershell.exe -NoProfile -File "{{SKILL_DIR}}/scripts/form-edit.ps1" -FormPath "<путь>" -JsonPath "<путь>"
```
...
`FormPath` указывает на файл `Ext/Form.xml`. Модуль формы (`Module.bsl`) **обязательно** располагается в подкаталоге `Form/` внутри `Ext/`:

```
Forms/Форма/
└── Ext/
    ├── Form.xml       (определение формы — FormPath)
...
`/form-edit` модифицирует только `Form.xml` и не затрагивает `Module.bsl`. При добавлении обработчиков событий (`on`) соответствующие процедуры нужно дописать в `Ext/Form/Module.bsl` вручную.
