...
```powershell
powershell.exe -NoProfile -File "{{SKILL_DIR}}/scripts/epf-build.ps1" <параметры>
```
...
powershell.exe -NoProfile -File "{{SKILL_DIR}}/scripts/epf-build.ps1" -InfoBasePath "C:\Bases\MyDB" -SourceFile "src/МояОбработка.xml" -OutputFile "build/МояОбработка.epf"

# Серверная база
powershell.exe -NoProfile -File "{{SKILL_DIR}}/scripts/epf-build.ps1" -InfoBaseServer "srv01" -InfoBaseRef "MyDB" -UserName "Admin" -Password "secret" -SourceFile "src/МояОбработка.xml" -OutputFile "build/МояОбработка.epf"
```
