...
```powershell
# Режим JSON DSL
powershell.exe -NoProfile -File "{{SKILL_DIR}}/scripts/form-compile.ps1" -JsonPath "<json>" -OutputPath "<Form.xml>"

# Режим from-object (объект и purpose выводятся из OutputPath; Document и Catalog)
powershell.exe -NoProfile -File "{{SKILL_DIR}}/scripts/form-compile.ps1" -FromObject -OutputPath "<.../TypePlural/ObjectName/Forms/FormName/Ext/Form.xml>"
```

## Структура каталогов формы

`OutputPath` указывает на файл `Ext/Form.xml`. Модуль формы (`Module.bsl`) **обязательно** располагается в подкаталоге `Form/` внутри `Ext/`, а не непосредственно в `Ext/`.

Правильная структура 1С XML-исходников для формы:

```
Forms/Форма/
├── Форма.xml          (метаданные формы — необязательно, если есть Forms/Форма.xml)
└── Ext/
    ├── Form.xml       (определение формы — OutputPath)
...
`/form-compile` генерирует только `Form.xml`. Модуль формы (`Module.bsl`) создаётся навыком `/form-add` и должен находиться по пути `Ext/Form/Module.bsl`.
