...
```powershell
powershell.exe -NoProfile -File "{{SKILL_DIR}}/scripts/db-run.ps1" <параметры>
```
...
```powershell
# Простой запуск
powershell.exe -NoProfile -File "{{SKILL_DIR}}/scripts/db-run.ps1" -InfoBasePath "C:\Bases\MyDB" -UserName "Admin"

# Запуск с обработкой
powershell.exe -NoProfile -File "{{SKILL_DIR}}/scripts/db-run.ps1" -InfoBasePath "C:\Bases\MyDB" -UserName "Admin" -Execute "C:\epf\МояОбработка.epf"

# Открыть по навигационной ссылке
powershell.exe -NoProfile -File "{{SKILL_DIR}}/scripts/db-run.ps1" -InfoBasePath "C:\Bases\MyDB" -UserName "Admin" -URL "e1cib/data/Справочник.Номенклатура"

# Серверная база с параметром запуска
powershell.exe -NoProfile -File "{{SKILL_DIR}}/scripts/db-run.ps1" -InfoBaseServer "srv01" -InfoBaseRef "MyDB" -UserName "Admin" -Password "secret" -CParam "ЗапуститьОбновление"
```
