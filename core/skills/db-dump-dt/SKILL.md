...
```powershell
powershell.exe -NoProfile -File "{{SKILL_DIR}}/scripts/db-dump-dt.ps1" <параметры>
```
...
powershell.exe -NoProfile -File "{{SKILL_DIR}}/scripts/db-dump-dt.ps1" -InfoBasePath "C:\Bases\MyDB" -UserName "Admin" -OutputFile "C:\backup\base.dt"

# Серверная база
powershell.exe -NoProfile -File "{{SKILL_DIR}}/scripts/db-dump-dt.ps1" -InfoBaseServer "srv01" -InfoBaseRef "MyApp_Dev" -UserName "Admin" -Password "secret" -OutputFile "base.dt"
```
